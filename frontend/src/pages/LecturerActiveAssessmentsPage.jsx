...
import { getActiveAssessments, getPlagiarismAlerts } from "../services/assessmentService"
import { toast } from "react-toastify"

const LecturerActiveAssessmentsPage = () => {
  useEffect(() => {
    document.title = "Active Assessments"
  }, [])
  ...
  const [selectedTimeframe, setSelectedTimeframe] = useState("all") // Changed default to 'all'
  ...
  const fetchActiveAssessmentsData = async () => {
    setLoading(true)
    setError(null)
    try {
      const [activeData, alertsData] = await Promise.all([getActiveAssessments(), getPlagiarismAlerts()])
      setActiveAssessments(activeData)
  ...
  const filterAssessments = (assessments) => {
    return assessments.filter((assessment) => {
      ...
      const matchesCourse = selectedCourse === "all" || assessment.courseId === Number.parseInt(selectedCourse) // Assuming courseId is numeric
      ...
      if (selectedTimeframe === "week") {
        ...
        matchesTimeframe = startDate >= oneWeekAgo
      } else if (selectedTimeframe === "month") {
        const oneMonthAgo = new Date(now.setMonth(now.getMonth() - 1))
        matchesTimeframe = startDate >= oneMonthAgo
      } else if (selectedTimeframe === "semester") {
        // This would require more complex logic based on actual semester dates
        // For now, let's assume 'semester' means within the last 6 months
        const sixMonthsAgo = new Date(now.setMonth(now.getMonth() - 6))
        matchesTimeframe = startDate >= sixMonthsAgo
      }
  ...
  const filteredActiveAssessments = filterAssessments(activeAssessments)
          ...
          >
            ...
            <div className="flex-1 flex flex-col overflow-y-auto">
              <nav className="flex-1 px-2 py-4 space-y-1">
                ...
                >
                  <FiTarget className="mr-3 h-5 w-5 text-[#00BFA5]" /> Active Assessments
                </Link>
                <Link
          ...
          <div className="md:ml-64 flex-1">
                  ...
                  <div className="flex flex-wrap gap-4">
                    <div>
                      ...
                      >
                        ...
                        <option value="1">CSC 401</option> {/* Assuming course IDs */}
                    ...
                    <div className="flex items-end">
                      <button
                        onClick={fetchActiveAssessmentsData} // Re-fetch or apply filters
            ...
            <main className="p-4 sm:p-6 lg:p-8">
              <div className="max-w-4xl mx-auto">
                ...
                <h1 className="text-3xl font-bold mb-6 text-gray-900 dark:text-white">Active Assessments</h1>
                <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 mb-8">
                  {error ? (
                    ...
                    <div className="overflow-x-auto">
                      <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                          {filteredActiveAssessments.map((assessment) => (
                            <tr key={assessment.id}>
                              ...
                              <td className="px-4 py-4 whitespace-nowrap text-sm font-medium">
                                <Link
                                  to={`/lecturer/assessments/${assessment.id}/grade`}
                    ...
                    <div className="text-center text-gray-500 dark:text-gray-400 py-8">
                      No active assessments found matching your criteria.
...
export default LecturerActiveAssessmentsPage
